...
from package_manifest import HOOK_NAMES, SKILL_NAMES
from state_store import read_state, validate_progress_file
...
def main() -> int:
    ...
    workspace_root = (
        normalize_input_path(sys.argv[1])
        if len(sys.argv) > 1
        else Path(os.environ.get('PWD') or os.getcwd())
    )
    codex_home_value = sys.argv[2] if len(sys.argv) > 2 else os.environ.get('CODEX_HOME') or (Path.home() / '.codex')
    agents_home_value = sys.argv[3] if len(sys.argv) > 3 else os.environ.get('AGENTS_HOME') or (Path.home() / '.agents')

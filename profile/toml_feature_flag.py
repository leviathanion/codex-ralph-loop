...
from ralph_core.storage import atomic_write_text
...
@dataclass(frozen=True)
class AssignmentValueState:
    multiline_string: MultilineStringState
...
def assignment_range(lines: list[str], start: int) -> AssignmentRange:
    value_start = lines[start].find('=')
    if value_start == -1:
        return AssignmentRange(start=start, end=start + 1, closed=True)

    state = advance_assignment_value_state(
        lines[start],
        AssignmentValueState(multiline_string='none'),
        start_index=value_start + 1,
    )
    if assignment_value_closed(state):
        return AssignmentRange(start=start, end=start + 1, closed=True)

    for idx in range(start + 1, len(lines)):
        state = advance_assignment_value_state(lines[idx], state)
        if assignment_value_closed(state):
            return AssignmentRange(start=start, end=idx + 1, closed=True)

    # Trade-off: do not try to "repair" an unterminated multiline value by deleting everything to
    # EOF. Mark it unclosed so the section editor leaves the original text intact and lets TOML
    # validation fail without mutating unrelated lines after a malformed user edit.
    return AssignmentRange(start=start, end=start + 1, closed=False)


def assignment_value_closed(state: AssignmentValueState) -> bool:
    return state.multiline_string == 'none' and not state.bracket_stack


def advance_assignment_value_state(
    line: str,
    state: AssignmentValueState,
    *,
    start_index: int = 0,
) -> AssignmentValueState:
    # Trade-off: this is not a full TOML parser. It only identifies the physical extent of the
    # codex_hooks value so replacing a wrong typed value does not leave array/table tail lines
    # behind. The final candidate is still validated by tomllib before writing.
    multiline_string = state.multiline_string
    bracket_stack = list(state.bracket_stack)
    in_basic_string = False
    in_literal_string = False
    escaped = False
    idx = start_index
    ...
    while idx < len(line):
        ...
        if char == '#':
            break
        if line.startswith('"""', idx):
            multiline_string = 'basic'
            idx += 3
            continue
        if line.startswith("'''", idx):
            multiline_string = 'literal'
            idx += 3
            continue
        if char == '"':
            in_basic_string = True
    ...
    return AssignmentValueState(
        multiline_string=multiline_string,
        bracket_stack=tuple(bracket_stack),
    )

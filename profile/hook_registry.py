...
PYTHON_EXECUTABLE_PATTERN = re.compile(r'^python3(?:\.\d+)?$')
...
class StopHookInspection:
    ...
    @property
    def bounded_timeout_count(self) -> int:
        return sum(1 for match in self.matches if match.timeout == STOP_HOOK_TIMEOUT_SECONDS)
...
def is_python_executable(command_name: str) -> bool:
    return PYTHON_EXECUTABLE_PATTERN.fullmatch(Path(command_name).name) is not None
...
def parse_stop_command(stop_command: str) -> ParsedStopCommand | None:
    ...
    if parts is not None and len(parts) == 2 and is_python_executable(parts[0]):
        return ParsedStopCommand(
            script_path=Path(parts[1]).expanduser().resolve(strict=False),
            shell_safe=True,
        )

    legacy_match = re.fullmatch(r'(?:\S*/)?python3(?:\.\d+)?\s+(.+)', trimmed)
    ...
    for entry_index, entry in enumerate(registry.get('hooks', {}).get('Stop', [])):
        for hook_index, hook in enumerate(entry.get('hooks', [])):
            ...
            matches.append(StopHookMatch(
                ...
                timeout=raw_timeout if type(raw_timeout) is int else None,
            ))
...
def stop_hook_registered(
    ...
    *,
    ...
    if require_shell_safe or require_bounded_timeout:
        return any(
            ((not require_shell_safe) or match.shell_safe)
            and ((not require_bounded_timeout) or match.timeout == STOP_HOOK_TIMEOUT_SECONDS)
...
def register_stop_hook(path: Path, stop_command: str) -> str:
    ...
    inspection = inspect_stop_hook_registration(registry, stop_command)
    if (
        inspection.equivalent_count == 1
        and inspection.shell_safe_count == 1
        and inspection.bounded_timeout_count == 1
    ):
        return 'unchanged'
        ...
        for entry_index, entry in enumerate(registry.get('hooks', {}).get('Stop', [])):
            ...
            for hook_index, hook in enumerate(entry_hooks):
                ...
                if entry_index == preferred.entry_index and hook_index == preferred.hook_index:
                    if preferred.shell_safe and preferred.timeout == STOP_HOOK_TIMEOUT_SECONDS:
                        remaining_hooks.append(hook)
                    else:
                        updated_hook = dict(hook)
                        if not preferred.shell_safe:
                            updated_hook['command'] = stop_command
                        if preferred.timeout != STOP_HOOK_TIMEOUT_SECONDS:
                            # Trade-off: Ralph manages this one profile hook, so reinstall repairs
                            # legacy registrations to a bounded timeout instead of preserving a
                            # default ten-minute Stop-hook wait.
                            updated_hook['timeout'] = STOP_HOOK_TIMEOUT_SECONDS
